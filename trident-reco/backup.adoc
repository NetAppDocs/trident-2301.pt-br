...
NOTE: Para saber mais sobre as tecnologias de replicação do ONTAP, consulte o https://docs.netapp.com/us-en/ontap/concepts/snapshot-copies-concept.html["Documentação do ONTAP"^].



=== Replicação de máquinas virtuais de storage (SVM) da SnapMirror

Use https://docs.netapp.com/us-en/ontap/concepts/snapmirror-disaster-recovery-data-transfer-concept.html["SnapMirror"^] o para replicar um SVM completo, que inclui suas configurações e volumes. Em caso de desastre, você pode ativar o SVM de destino do SnapMirror para começar a fornecer dados. Você pode voltar para o primário quando os sistemas forem restaurados.
